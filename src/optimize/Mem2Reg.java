...
import static utils.Configs.mem2reg;
...
public class Mem2Reg {
    ...
    public void start() {
        if (mem2reg) {
            // 遍历每一个函数
            for (Function function1 : IrModule.getInstance().getFunctions()) {
    ...
    private void process() {
        // create alloca list
        entryBlock = function.getBlocks().getFirst();
        for (Instruction instr  : entryBlock.getInstList()) {
    ...
    private void dealOneBlock(Alloca alloca) {
        ...
        // 遍历所有的指令
        LinkedList<Instruction> instructions = new LinkedList<>(onlyBlock.getInstList());
    ...
    private void sweepBlock(BasicBlock block) {
        ...
        ArrayList<Instruction> instructions = new ArrayList<>(block.getInstList());
        for (Instruction instruction : instructions) {
        ...
        LinkedList<Instruction> instructions1 = block.getInstList();
        // 但如果 instructions1 是 LinkedList，get(i) 效率极低。
        // 为了安全起见，这里也可以改为操作副本，或者保持原样（因为是从后往前删，索引影响较小，但 LinkedList get(i) 确实慢）。
        // 考虑到这是 LinkedList，get(i) 是 O(N)，总复杂度 O(N^2)。
        // 更好的做法是转为 ArrayList 处理，或者使用迭代器。
        // 但为了修复报错，上面的修改已经足够。
        for (int i = instructions1.size()-1 ; i >= 0 ; i--){
    ...
    private void renamePhiNode() {
        ...
        while (!bbStack.isEmpty()) {
            ...
            // 遍历当前块的所有指令
            int i = 0;
            LinkedList<Instruction> instructions = new LinkedList<>(currentBlock.getInstList());
            ...
            while (i < instructions.size()) {
                ...
                if (instruction instanceof Load load) {
                    if (load.getAddr() instanceof Alloca) {
                        instruction.replaceAllUsesWith(variableVersion.get((Alloca) load.getAddr()));
                        instruction.dropAllOperands();
                        instruction.eraseFromParent();
                    }
            ...
            for (BasicBlock successor : currentBlock.getSuccessors()) {
                instructions = new LinkedList<>(successor.getInstList());
                i = 0;
                while (instructions.get(i) instanceof Phi) {
